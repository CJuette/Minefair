...
class Game {
  ...
  mount(gameElement) {
    ...
    for (let y = 0; y < this.height; y++) {
      ...
      for (let x = 0; x < this.width; x++) {
        ...
        const rippleEffect = document.createElement('ons-ripple');
        rippleEffect.setAttribute('color', '#fff');
        rippleEffect.setAttribute('background', '#e0e0e0');
        cell.appendChild(rippleEffect);
  ...
  refresh() {
    ...
    switch (this.state) {
      case State.PLAYING:
        message = `${this.numFlags}/${this.numMines}`;
...
function newGame(event) {
  ...
  const gameElement = document.getElementById('playingfield');
